...
// Pins - Outputs
int triggerPin = 2;
// MCP pins are 11 and 13
//eg: MCP4821 MCP(11,13);
...
void updateControl() {
  ...
  //inversion setting
  inv_knob = mozziAnalogRead(slideCVPin);
  inv = inv_knob / 64;
...
void audioOutput(const AudioOutput f) {
  ...
  MCP.write(f.l() + MOZZI_AUDIO_BIAS, f.r() + MOZZI_AUDIO_BIAS);
}
...
void LED() {
  ...
  analogWrite(ledPin1, 255 - LEDR);
  analogWrite(ledPin2, 255 - LEDG);
  analogWrite(ledPin3, 255 - LEDB);
}
